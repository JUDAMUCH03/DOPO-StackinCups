...
public class Tower {
    ...
    public int height() {
        ...
        for (StackableItem item : items) {
            if (item instanceof Cup) {
                ...
                int lidTopY = prevItemTopY - item.getHeightCm();
    ...
    public int[] lidedCups() {
        ...
        for (StackableItem item : items) {
            if (item instanceof Cup && lidExists(item.getNumber())) {
    ...
    private Cup findCup(int number) {
        for (StackableItem item : items) {
            if (item instanceof Cup && item.getNumber() == number) return (Cup) item;
        }
        return null;
    }
    ...
    private Lid findLid(int number) {
        for (StackableItem item : items) {
            if (item instanceof Lid && item.getNumber() == number) return (Lid) item;
        }
        return null;
    }
        ...
        for (StackableItem item : items) {
            if (item instanceof Cup) {
                ...
                if (cupTopYMap.containsKey(lid.getNumber())) {
                    lidBottomY = Math.min(prevItemTopY, cupTopYMap.get(lid.getNumber()));
                }
                int lidTopY = lidBottomY - lid.getHeightCm() * SCALE;
