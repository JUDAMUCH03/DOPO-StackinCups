...
public class Tower {
     ...
     * todas las tazas quepan en el peor caso
    ...
    /*
     * Notifica a todas las tazas fragiles para que verifiquen si deben romperse.
     */
    private void checkFragile() {
        ArrayList<StackableItem> copy = new ArrayList<>(items);
        for (StackableItem item : copy) {
            if (item.isFragile()) {
                ((FragileCup) item).validateIntegrity(this);
            }
        }
    }

    /*
     * Permite a los elementos del paquete leer la lista de items (uso preferido: solo lectura).
     * Para mutar la lista usar addItem(), addItemAt() o removeAllLids().
     */
    ...
    /*
     * Agrega un elemento al final de la pila de la torre.
     */
    ...
    /*
     * Agrega un elemento en la posicion indicada de la pila.
     */
    void addItemAt(int index, StackableItem item) {
        this.items.add(index, item);
    }

    /*
     * Elimina todas las tapas de la torre, haciendolas invisibles primero.
     */
    void removeAllLids() {
        for (StackableItem item : this.items) {
            if (item.isLid()) item.makeInvisible();
        }
        this.items.removeIf(StackableItem::isLid);
    }

    /*
     * Indica si existe alguna taza en una posicion fisica mas baja que el elemento indicado.
     * Usado por HierarchicalCup para determinar si puede ser removida de la base.
     */
    boolean hasCupBelow(StackableItem target) {
        for (StackableItem item : this.items) {
            if (item == target) break;
            if (item.isCup()) return true;
        }
        return false;
    }
